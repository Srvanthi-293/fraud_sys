...
/**
 * Seeds the H2 database with sample customers and transactions on startup.
 * No manual data entry required — everything is ready when the app starts.
 */
@Component
public class DataLoader implements CommandLineRunner {
    ...
    public void run(String... args) {
        ...
        // ── Transactions ───────────────────────────────────────────────────
        // id | customerId | amount   | payee           | newPayee | hour | location  | timestamp

        // Sai — T101: small amount, known payee, home city → SAFE (0 pts)
        transactionRepository.save(new Transaction(
                101L, 1L, 2000, "Grocery Store", false, 13, "Chennai",
                LocalDateTime.of(2025, 6, 5, 13, 0)));

        // Sai — T102: high amount, new payee, 3 AM, Mumbai → FRAUD (90 pts)
        transactionRepository.save(new Transaction(
                102L, 1L, 90000, "Unknown Payee", true, 3, "Mumbai",
                LocalDateTime.of(2025, 6, 5, 3, 0)));

        // Rahul — T103: high amount, known payee, home city → REVIEW (50 pts: high+baseline)
        transactionRepository.save(new Transaction(
                103L, 2L, 75000, "Amazon", false, 14, "Bangalore",
                LocalDateTime.of(2025, 6, 5, 14, 0)));

        // Priya — T104: high amount, new payee, 2 AM, Delhi → FRAUD (90 pts)
        transactionRepository.save(new Transaction(
                104L, 3L, 65000, "Unknown Payee", true, 2, "Delhi",
                LocalDateTime.of(2025, 6, 5, 2, 0)));

        // Rahul — T105: safe transaction, home city
        transactionRepository.save(new Transaction(
                105L, 2L, 5000, "Swiggy", false, 12, "Bangalore",
                LocalDateTime.of(2025, 6, 5, 12, 0)));

        // Priya — T106: odd hour only
        transactionRepository.save(new Transaction(
